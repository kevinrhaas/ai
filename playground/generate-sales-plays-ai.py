...
def main():
    sample_sales_play = read_file('sample-sales-play.txt')


    if len(sys.argv) == 3 and sys.argv[1] == "--file":
        input_file = sys.argv[2]
        try:
            json_data = json.loads(read_file(input_file))
        ...
        with open(output_file_name, "w") as output_file:
            for play in json_data['plays']:
                ...
                print(f"Generated Sales Play: {play['name']}\n\n{result}\n\n\n")
                output_file.write(f"Generated Sales Play: {play['name']}\nSales Play Summary Prompt: {play['summary']}\n\n{result}\n\n\n")
    else:
        if len(sys.argv) != 3:
