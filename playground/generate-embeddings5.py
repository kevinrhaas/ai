...
def read_docx_to_array(file_path):
    ...
    # Define a list of styles to keep
    styles_to_keep = ['Normal']

    # Iterate through each paragraph in the document
    for para in doc.paragraphs:
        # Check if the paragraph style is in the list of styles to keep
        if para.style.name in styles_to_keep and para.text.strip():
            # Add the paragraph text to the list if it is not empty and in the styles to keep
...
# Calculate cluster centers as the mean of points in each cluster (excluding noise points labeled as -1)
cluster_centers = []
for label in unique_labels:
    if label != -1:
        cluster_centers.append(reduced_embeddings[clusters == label].mean(axis=0))
...
for i, label in enumerate(unique_labels):
    ...
    original_indices = np.where(clusters == label)[0]
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], label=f'Cluster {label}', color=col)
...
# Plotting the cluster centers with matching colors (excluding noise points)
for i, center in enumerate(cluster_centers):
    plt.scatter(center[0], center[1], s=300, color=colors[i % len(colors)], marker='X', edgecolor='black', linewidth=2, label=f'Centroid {i}')
...
if len(np.unique(clusters)) > 1 and np.any(clusters != -1):
    score = silhouette_score(reduced_embeddings[clusters != -1], clusters[clusters != -1])
...
clustered_docs = {label: [] for label in unique_labels if label != -1}
distances_to_center = {label: [] for label in unique_labels if label != -1}
...
def print_top_n_closest_texts(clustered_docs, top_n=10):
...
# Example usage
print_top_n_closest_texts(clustered_docs, top_n=10)
