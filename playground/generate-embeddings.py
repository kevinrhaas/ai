...
# Download the punkt tokenizer models if not available
# nltk.download('punkt')
...
if use == 'openai':
    ...
    # Convert each embedding to the OpenAI format
...
# Calculate n_components based on the shape of the embeddings
...
# Alternatively, you can use UMAP for dimensionality reduction
# reducer = umap.UMAP(n_components=2)
# reduced_embeddings = reducer.fit_transform(embeddings)



# Calculate the number of clusters based on the number of samples
num_clusters = max(2, min(5, reduced_embeddings.shape[0] - 1))
...
# Assume 'reduced_embeddings' is your data after PCA, and 'labels' are the cluster labels
#reduced_embeddings2 = np.array(reduced_embeddings)
labels = kmeans.labels_

# Create a scatter plot
plt.figure(figsize=(10, 7))
unique_labels = np.unique(labels)
colors = plt.cm.tab10(np.linspace(0, 1, len(unique_labels)))  # Generate a list of colors

for i, label in enumerate(unique_labels):
    cluster_points = reduced_embeddings[labels == label]
    original_indices = np.where(labels == label)[0]  # Get original indices of points in this cluster
...
def extract_key_phrases(docs, top_n=5):
    # Join all documents in the cluster
    text = " ".join(docs)
...
for label, docs in clustered_docs.items():
    ...
    cluster_summaries[label] = key_phrases

# Print summaries
for label, summary in cluster_summaries.items():
    print(f"Cluster {label}: {', '.join(summary)}")


####sentences

# Document reduction - Summarization using Sumy
def extract_key_sentences(docs, sentence_count=2):
    text = " ".join(docs)
