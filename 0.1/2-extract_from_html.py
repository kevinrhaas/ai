...
parser.add_argument('--truncate', help='Do the truncate option')
...
if any(x in args.truncate for x in ['Y','y','yes','Yes']):
    ...
    if any(x in user_input for x in ['Y','y','yes','Yes']):
        ...
        print("Truncating feature table")
        cur.execute("TRUNCATE TABLE feature")
        conn.commit()
...
for href in url_to_process:
    ...
    if href.startswith('http'):
        ...
        for data in soup.find_all(["h1", "h2", "h3", "h4", "h5", "h6", "p", "ul", "ol", "a", "pre"]):
            ...
            cur = conn.cursor()
            # print(id, element, para, href)
            cur.execute("INSERT INTO feature(id, element, para, href, process_time, process_name, parent_url) VALUES(%s, %s, %s, %s, NOW(), %s, %s)", (id, element, para, href, args.name, args.url))
            conn.commit()
